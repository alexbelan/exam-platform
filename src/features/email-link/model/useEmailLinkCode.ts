...
export function useEmailLinkCode(email: string) {
  const toast = useToastClient();
  const { fetch } = useUserSession();
  const { linkEmailWithCode, resendCode } = useAsyncEmailLink();
  ...
  const handleSubmit = async (
    ...
    try {
      await linkEmailWithCode(email, codeValue, passwordValue);
      ...
      toast.add({
        severity: "success",
        summary: "Успешно",
        detail: "Email успешно привязан к аккаунту",
      });
      ...
      const errorMessage =
        (error as Error)?.message || "Произошла ошибка при привязке email";
