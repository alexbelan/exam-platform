...
export function useAdminUsersCatalog(emit: {
  (event: "create"): void;
  ...
  const { users, pagination, loading, refresh, cacheKey } =
    useUsersTable(tableFilters);

  const handleView = async (user: User) => {
    try {
      await trpc.users.getById.query({
        id: user.id.toString(),
      });
      emit("view", user);
    } catch (error) {
  ...
  const exportUsers = () => {
    console.warn("Экспорт пользователей");
