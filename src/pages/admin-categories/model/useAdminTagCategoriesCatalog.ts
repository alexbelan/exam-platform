...
import { useCategoriesTable } from "@features/categories-table";
import { extractErrorMessage } from "@shared/utils";
import type { AdminTagCategoriesCatalogFilters } from "./types";
import type {
  CategoryTableItem,
  CategoriesTableFilters,
} from "@features/categories-table";

export function useAdminTagCategoriesCatalog(emit: {
  ...
  const { categories, pagination, loading, columns, refresh, cacheKey } =
    useCategoriesTable(
      tableFilters,
      (event: { page: number; rows: number }) => {
  ...
  const handleDelete = async (category: CategoryTableItem) => {
    confirm.require({
      ...
      accept: async () => {
        try {
          await trpc.tagCategories.delete.mutate({
